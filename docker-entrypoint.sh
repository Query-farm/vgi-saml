...
#
# Dispatch the single vgi-saml image into one of its transports:
...
#
# The worker is stateless (verify-only, no key store, no egress), so there is no
...
case "${1:-http}" in
  http)
    ...
    exec saml-worker --http "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    exec saml-worker --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec saml-worker "$@"
