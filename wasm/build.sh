#!/usr/bin/env bash
# Build the browser (`worker:`) build of the saml VGI worker: saml-wasm compiled
...
LIB="target/$TARGET/release/libsaml_wasm.a"
...
# every allocation on a global lock, so concurrent serve threads convoy on it.
#
# STACK_SIZE / DEFAULT_PTHREAD_STACK_SIZE: emscripten defaults to a 64 KiB stack,
# which the crypto + Arrow encoders overflow — an overflow surfaces as a bare
# "memory access out of bounds" with no console output. INITIAL_MEMORY must cover
# main stack + every pthread stack up front, or startup stalls growing shared
# memory while the pool is spawning.
echo "==> emcc link"
